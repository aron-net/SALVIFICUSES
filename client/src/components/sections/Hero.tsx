...
const serviceDetails = [
  ...
  },
  {
    ...
    isHighlighted: true, // Set to true to test highlighted state
  },
  {
    title: "LANGUAGE",
...
export function Hero() {
  ...
  return (
    <section 
      id="home" 
      className="relative text-white bg-gradient-to-r from-blue-600 to-blue-600/80 pt-24 pb-20"
      style={{
    ...
    >
      <div className="absolute inset-0 bg-blue-700 opacity-90 z-0"></div>
        ...
        >
          <div className="flex flex-col gap-4">
            ...
            <div className="flex gap-4 mt-8 justify-center">
              <a href="#contact" className="px-8 py-4 bg-white text-blue-600 font-semibold rounded-md hover:bg-opacity-90 transition-all">
                Get Started
              </a>
              <a href="#services" className="px-8 py-4 border-2 border-white text-white font-semibold rounded-md hover:bg-white hover:text-blue-600 transition-all">
                Our Services
              </a>
            </div>
          </div>
      ...
      <div className="relative z-20 max-w-6xl mx-auto px-4 -mb-32">
        <motion.div className="grid grid-cols-1 md:grid-cols-5 gap-1 bg-white rounded-lg shadow-xl overflow-hidden">
          {serviceDetails.map((service, index) => (
            <div key={index} className="relative">
              <button 
                ...
                className={`w-full ${activeHighlight === index ? 'bg-blue-600 !text-white' : 'bg-white !text-gray-900'} p-6 hover:${activeHighlight === index ? 'bg-blue-700' : 'bg-gray-50'} transition-colors text-center cursor-pointer`}
              >
                <h3 className={`font-semibold ${service.isHighlighted ? '!text-white' : '!text-blue-600'} uppercase text-sm`}>{service.title}</h3>
                <p className={`${service.isHighlighted ? '!text-white/90' : '!text-gray-600'} text-xs mt-2`}>{service.shortDesc}</p>
              ...
              {activeService === index && (
                <motion.div 
                  ...
                  className={`${service.isHighlighted ? 'bg-blue-600 !text-white' : 'bg-white'} border-t border-gray-100 p-4 text-left shadow-lg`}
                >
                  <p className={`${service.isHighlighted ? '!text-white/90' : '!text-gray-900'} text-sm`}>
                    {service.longDesc}
                  </p>
                  <a 
                    href="#services" 
                    className={`mt-3 inline-flex items-center text-xs font-medium ${service.isHighlighted ? '!text-white hover:!text-white/80' : '!text-blue-600 hover:!text-blue-600/80'}`}
                  >
      ...
      <div className="relative bg-gradient-to-r from-gray-900 to-blue-600 py-24">
